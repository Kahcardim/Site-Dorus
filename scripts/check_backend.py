...
reviews = (ROOT / 'google-reviews.js').read_text(encoding='utf-8')
...
for marker in ('issueBridgeSession','validateBridgeSession','appointmentFingerprint','CacheService.getScriptCache()','MAX_BOOKINGS_PER_SLOT = 2','lock.waitLock(5000)'):
...
for marker in ('max.setDate(max.getDate() + 60)',"weekday === 7","phoneDigits.length < 10","calendar.getEvents(slot.start, slot.end)"):
...
if expected_place_id not in reviews: errors.append('google-reviews.js: Place ID esperado da D’orus ausente.')
if 'data.placeId !== EXPECTED_PLACE_ID' not in reviews: errors.append('google-reviews.js: resposta sem validação da ficha correta.')
if 'placeId: placeId' not in code: errors.append('Code.gs: resposta das avaliações não identifica o Place ID consultado.')
if errors:
